...
void CFloatingDragPreview::finishDragging()
{
	...
	{
		CDockAreaWidget* dockAreaWidget = dynamic_cast<CDockAreaWidget*>(d->Content);
		CDockWidget* dockWidget = dynamic_cast<CDockWidget*>(d->Content);
		CDockAreaTitleBar* titleBar = nullptr;

		if (dockAreaWidget != nullptr)
			titleBar = dockAreaWidget->titleBar();
		else
			titleBar = dockWidget->dockAreaWidget()->titleBar();

		d->DropContainer->dropWidget(d->Content, DockDropArea, d->DropContainer->dockAreaAt(QCursor::pos()));

		if (dockAreaWidget != nullptr && dockAreaWidget->dockWidgetsCount() == 1)
			dockAreaWidget->currentDockWidget()->runDockStateHandler(true);
		else if (dockWidget != nullptr)
			dockWidget->runDockStateHandler(true);

		if (titleBar != nullptr && titleBar->tabBar()->count() > 0)
			titleBar->onCurrentTabChanged(0);
	}
