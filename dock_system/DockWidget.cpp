...
struct DockWidgetPrivate
{
	...
	CDockWidget::eViewType view_type = CDockWidget::VIEW_SETTING;
	/**
...
//============================================================================
CDockWidget::CDockWidget(const QString &title, eViewType view_type, QWidget *parent) :
	QFrame(parent),
...
void CDockWidget::toggleViewInternal(bool Open)
{
	...
	{
		if (allow_close_handler_ && !allow_close_handler_())
			return;
...
void CDockWidget::resizeEvent(QResizeEvent*) {
	emit resize_event();
}
...
void CDockWidget::set_tab_color(QString color) {
	d->TabWidget->setStyleSheet("ads--CDockWidgetTab[activeTab=""true""]{ background:" + color + ";border-color:" + color+ ";}");
}
void CDockWidget::set_tab_rgb(int r, int g, int b, int a) {
	tab_color_ = QColor(r, g, b, a);
}
void CDockWidget::set_border_color(QString color) {
	border_color_str_ = color;
}
QString CDockWidget::get_border_color() {
	return border_color_str_;
}

QColor CDockWidget::get_tab_color() {
	return tab_color_;
}

void CDockWidget::set_allow_close_handler(const std::function<bool()> &handler) {
	allow_close_handler_ = handler;
}

void CDockWidget::set_view_type(CDockWidget::eViewType viewtype)
{
	d->view_type = viewtype;
}

CDockWidget::eViewType CDockWidget::get_view_type() 
{
	return d->view_type;
}

void CDockWidget::setDockStateHandler(std::function<void(bool)> handler)
{
	DockStateHandler = handler;
}

std::function<void(bool)> CDockWidget::dockStateHandler()
{
	return DockStateHandler;
}

void CDockWidget::runDockStateHandler(bool state)
{
	if (DockStateHandler != nullptr)
	{
		DockStateHandler(state);
	}
}

void CDockWidget::set_close_handler(std::function<void()> handler)
{
	CloseHandler = handler;
}

void CDockWidget::runCloseHandler()
{
...
void CDockWidget::set_play_pause_button_handler(std::function<void(bool)> handler)
{
	PlayPauseBtnHandler = handler;
}


void CDockWidget::set_setting_button_handler(std::function<void(bool)> handler)
{
	SettingBtnHandler = handler;
}


void CDockWidget::set_retsetting_button_handler(std::function<void()> handler)
{
	ResettingBtnHandler = handler;
}


void CDockWidget::set_capture_button_handler(std::function<void()> handler)
{
	CaptureBtnHandler = handler;
}

void CDockWidget::set_tab_change_handler(std::function<void(bool)> handler)
{
	TabChangeHandler = handler;
}

void CDockWidget::runPlayPauseHandler(bool state)
{
	PlayState = state;
	if (PlayPauseBtnHandler != nullptr)
	{
		PlayPauseBtnHandler(PlayState);
	}
}

void CDockWidget::runSettingHandler(bool state)
{
	SettingState = state;
	if (SettingBtnHandler != nullptr)
	{
		SettingBtnHandler(SettingState);
	}
}

void CDockWidget::runResettingHandler()
{
	if (ResettingBtnHandler != nullptr)
	{
		ResettingBtnHandler();
	}
}

void CDockWidget::runCaptureHandler()
{
	if (CaptureBtnHandler != nullptr)
	{
		CaptureBtnHandler();
	}
}

void CDockWidget::runTabChangeHandler(bool state)
{
	if (TabChangeHandler != nullptr)
	{
		TabChangeHandler(state);
	}
}

void CDockWidget::playState(bool state)
{
	PlayState = state;
}

bool CDockWidget::playState()
{
	return PlayState;
}

void CDockWidget::settingState(bool state)
{
	SettingState = state;
}

bool CDockWidget::settingState()
{
	return SettingState;
}
