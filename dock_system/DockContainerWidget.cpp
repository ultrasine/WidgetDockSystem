...
void refresh(QWidget* window) {

	if (CDockManager::testConfigFlag(CDockManager::CustomFloatingTitlebar))
		return;
	...
	if (window->isMaximized()) {
		...
		window->resize(rect.width() - 1, rect.height() - 1);
...
public:
	...
	std::function<QList<int>(const QList<int> &)> adjust_size_handler_;
	 ...
	 */
	bool dropIntoSection(CFloatingDockContainer* FloatingWidget,
	 ...
	 */
	bool dropIntoCenterOfSection(CFloatingDockContainer* FloatingWidget,
	...
	void adjust_size_handler(const std::function<QList<int>(const QList<int>&)>& handler) {
		adjust_size_handler_ = handler;
	}
	/**
	...
	void adjustSplitterSizesOnInsertion(QSplitter* Splitter, qreal LastRatio = 1.0)
	{
		...
		if(adjust_size_handler_)
			SplitterSizes = adjust_size_handler_(SplitterSizes);
...
//============================================================================
bool DockContainerWidgetPrivate::dropIntoCenterOfSection(
	CFloatingDockContainer* FloatingWidget, CDockAreaWidget* TargetArea)
{
	if (!CDockManager::testConfigFlag(CDockManager::DropToCenter))
		return false;
...
//============================================================================
bool DockContainerWidgetPrivate::dropIntoSection(CFloatingDockContainer* FloatingWidget,
		CDockAreaWidget* TargetArea, DockWidgetArea area)
{
	...
	{
		return dropIntoCenterOfSection(FloatingWidget, TargetArea);
	}
...
void DockContainerWidgetPrivate::moveToNewSection(QWidget* Widget, CDockAreaWidget* TargetArea, DockWidgetArea area)
{
	...
	NewDockArea->set_min_area(TargetArea->min_area_handler_);
	auto InsertParam = internal::dockAreaInsertParameters(area);
	QSplitter* TargetAreaSplitter = internal::findParent<QSplitter*>(TargetArea);
	int AreaIndex = TargetAreaSplitter->indexOf(TargetArea);
	auto Sizes = TargetAreaSplitter->sizes();
	...
	CDockWidget* Dockwidget, CDockAreaWidget* TargetDockArea)
{
	...
	CDockAreaWidget* NewDockArea = new CDockAreaWidget(DockManager, _this);
	NewDockArea->set_min_area(TargetDockArea->min_area_handler_);
	NewDockArea->addDockWidget(Dockwidget);
	auto InsertParam = internal::dockAreaInsertParameters(area);
	...
	d(new DockContainerWidgetPrivate(this))
{
	...
	if (d->isFloating)
	   ...
	   d->Layout->setContentsMargins(6, 6, 6, 6);
...
void CDockContainerWidget::removeDockWidget(CDockWidget* Dockwidget)
{
	...
	if (Area)
	{
		...
		if (Area->dockWidgetsCount() == 0 && d->DockAreas.count() > 1)
	...
	const QPoint& TargetPos)
{
	...
	if (DockArea)
	{
		if (!DockArea->allow_drop())
			return;
		DockArea->show_close_btn();
		...
		{
            ADS_PRINT("Dock Area Drop Content: " << dropArea);
			Dropped = d->dropIntoSection(FloatingWidget, DockArea, dropArea);
		}
	}
	...
	if (Dropped)
	{
		...
		{
			SingleDroppedDockWidget->runDockStateHandler(true);
		}
	}
...
void CDockContainerWidget::dropWidget(QWidget* Widget, DockWidgetArea DropArea, CDockAreaWidget* TargetAreaWidget)
{
	...
	if (TargetAreaWidget)
	{
		ADS_PRINT("CDockContainerWidget::dropWidget moveToNewSection");
		if (!TargetAreaWidget->allow_drop())
			return;
		TargetAreaWidget->show_close_btn();
		d->moveToNewSection(Widget, TargetAreaWidget, DropArea);
	}
	else
	{
		
		ADS_PRINT("CDockContainerWidget::dropWidget moveToContainer");
...
void CDockContainerWidget::adjust_size_handler(const std::function<QList<int>(const QList<int>&)>& handler) {
	d->adjust_size_handler(handler);
}
