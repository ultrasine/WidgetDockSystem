...
struct DockManagerPrivate
{
	...
	std::function<QToolBar* (void *)> init_toolbar_;
	std::function<void(void *)> del_toolbar_;
	/**
...
{
	createRootSplitter();
	QMainWindow* window_t = qobject_cast<QMainWindow*>(parent);
	if (window_t)
	{
		window_t->setCentralWidget(this);
	}
	...
	CDockWidget* Dockwidget, CDockAreaWidget* DockAreaWidget)
{
	...
	if (DockAreaWidget != nullptr) {
		AreaOfAddedDockWidget->set_min_area(DockAreaWidget->min_area_handler_);
	}
	emit dockWidgetAdded(Dockwidget);
...
void CDockManager::set_toolbar_callback(const std::function<QToolBar* (void *)> &init_toolbar) {
	d->init_toolbar_ = init_toolbar;
}

QToolBar* CDockManager::create_toolbar(CFloatingDockContainer *impl){
	if (d->init_toolbar_) {
		return d->init_toolbar_(impl);
	}
	else
	{
		return nullptr;
...
void CDockManager::del_toolbar_callback(const std::function<void(void*)> &del_toolbar) {
	d->del_toolbar_ = del_toolbar;
}
void CDockManager::del_toolbar(CFloatingDockContainer *impl) {
	if (d->del_toolbar_) {
		d->del_toolbar_(impl);
	}
}

void CDockManager::adjust_size_handler(const std::function<QList<int>(const QList<int>&)>& handler) {
	CDockContainerWidget::adjust_size_handler(handler);
}
} // namespace ads
