...
struct FloatingDockContainerPrivate
{
	...
	QPoint DragStartPos;
	bool Hiding = false;
	title_bar_t* min_max_bar = nullptr;
	QToolBar* toolbar_ = nullptr;
	bool show_toolbar_ = false;
	...
	void setWindowTitle(const QString &Text)
	{
		...
		if(min_max_bar != nullptr)
			min_max_bar->set_window_tilte(Text);
#endif
	}
	...
	d(new FloatingDockContainerPrivate(this))
{
...
#else
	d->layout_ = new QBoxLayout(QBoxLayout::TopToBottom);
	...
	if (CDockManager::testConfigFlag(CDockManager::CustomFloatingTitlebar)) {
		...
		d->min_max_bar = new title_bar_t(this);
		d->min_max_bar->setObjectName("title_bar_");
		d->layout_->addWidget(d->min_max_bar);
	...
	d->layout_->addWidget(d->DockContainer);
#endif
...
CFloatingDockContainer::~CFloatingDockContainer()
{
	...
	if (d->DockManager)
	{
		d->DockManager->del_toolbar(this);
		...
		case WM_GETMINMAXINFO:
			{
				...
				if (this->isMaximized())
				{
					double dpi = screen()->devicePixelRatio();
					...
					p->ptMaxPosition.x = 0;
					p->ptMaxPosition.y = 0;
					p->ptMaxSize.x = rc.width() * dpi;
					p->ptMaxSize.y = rc.height() * dpi;
		...
		case WM_NCHITTEST:
			{
				*result = HTCLIENT;
				...
				else if (QRect(0, 0, w - 100, captionHeight).contains(pos))
				{
					*result = HTCAPTION;
				}
...
bool CFloatingDockContainer::eventFilter(QObject*, QEvent* event) {
	if (event->type() == QEvent::WindowStateChange) {
		d->min_max_bar->updata_icon();
	}
...
void CFloatingDockContainer::onDockAreasAddedOrRemoved()
{
	...
	if (TopLevelDockArea)
	{
		...
		if (CurrentWidget->get_view_type() == CDockWidget::VIEW_GRAPH)
			show_toolbar();
	}
	else
	{
...
void CFloatingDockContainer::show_toolbar() {

	if (!d->show_toolbar_) {
		if (d->toolbar_ == nullptr) {
			d->toolbar_ = d->DockManager->create_toolbar(this);
			d->layout_->insertWidget(1, d->toolbar_);
	    }

		if (d->toolbar_ != nullptr) {
			d->toolbar_->show();
			d->show_toolbar_ = true;
		}
	}		
}
void CFloatingDockContainer::hide_toolbar() {
	if (d->show_toolbar_) {
		if (d->toolbar_ != nullptr) {
			d->toolbar_->hide();
			d->show_toolbar_ = false;
		}
	}
}
} // namespace ads
