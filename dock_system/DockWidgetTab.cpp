...
struct DockWidgetTabPrivate
{
	...
	QPushButton* pull_down_btn = nullptr;
	...
	void create_pull_down_btn() 
	{
		pull_down_btn = new QPushButton();
		pull_down_btn->setObjectName("tab_pull_down_btn");
		_this->connect(pull_down_btn, SIGNAL(pressed()), SIGNAL(signls_pull_down()));
	}

	void show_pull_down_btn(bool is_show) 
	{
		if (pull_down_btn != nullptr) {
			pull_down_btn->setVisible(is_show);
		}
	}
	/**
...
void DockWidgetTabPrivate::createLayout()
{
	...
	internal::setToolTip(CloseButton, u8"关闭");
	...
	if (DockWidget->get_view_type() == CDockWidget::VIEW_GRAPH) {
		create_pull_down_btn();
		Layout->addWidget(pull_down_btn);
	}
...
bool DockWidgetTabPrivate::startFloating(eDragState DraggingState)
{
	...
	DockWidget->runDockStateHandler(false);
...
void CDockWidgetTab::mouseMoveEvent(QMouseEvent* ev)
{
	...
	if( !(d->DockArea->titleBar()->allow_move()) )
		return;
...
void CDockWidgetTab::contextMenuEvent(QContextMenuEvent* ev)
{
	...
	auto Action = Menu.addAction(u8"分离区域", this, SLOT(detachDockWidget()));
    Action->setEnabled(isDetachable);
	Menu.addSeparator();
	Action = Menu.addAction(u8"关闭当前区域", this, SIGNAL(closeRequested()));
	Action->setEnabled(isClosable());
	Menu.addAction(u8"关闭其他区域", this, SIGNAL(closeOtherTabsRequested()));
...
void CDockWidgetTab::set_pulldown_menu(QMenu* menu) {
	if (d->pull_down_btn != nullptr)
	{
		d->pull_down_btn->setMenu(menu);
	}
}
//==========================================================================
bool CDockWidgetTab::allow_move() {
	if (!d->DockArea->allow_move()) {
		//以后可能会有其他的限制情况
		if (d->DockArea->dockWidgetsCount() == 1) {
			return false;
		}
		else {
			return true;
